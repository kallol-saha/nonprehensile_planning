...
from pybullet_tools.pr2_primitives import get_grasp_gen, get_ik_ir_gen, get_stable_gen
...
def save_inverse_reachability(
    ...
    if has_gui():
        ...
        for gripper_from_base in gripper_from_base_list:
            handles.extend(
                draw_point(point_from_pose(gripper_from_base), color=(1, 0, 0))
            )
        wait_if_gui()
    return path
...
def create_inverse_reachability(
    ...
    while len(gripper_from_base_list) < num_samples:
        ...
        for attempt in range(max_attempts):
            robot_saver.restore()
            base_conf = next(
                uniform_pose_generator(robot, gripper_pose)
            )  # , reachable_range=(0., 1.)))
            ...
            grasp_conf = pr2_inverse_kinematics(
                robot, arm, gripper_pose
            )  # , nearby_conf=USE_CURRENT)
    ...
    return save_inverse_reachability(
        robot, arm, grasp_type, tool_link, gripper_from_base_list
    )
...
def create_inverse_reachability2(
    ...
    ik_ir_fn = get_ik_ir_gen(
        problem, max_attempts=max_attempts, learned=False, teleport=True
    )
