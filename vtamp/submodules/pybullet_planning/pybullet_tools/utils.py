...
from vtamp.submodules.motion_planners.meta import direct_path, solve
from vtamp.submodules.motion_planners.rrt_connect import birrt
...
def get_rigid_clusters(body):
    return get_connected_components(
        vertices=get_all_links(body), edges=get_adjacent_fixed_links(body)
    )
...
def adjust_path(robot, joints, path):
    ...
    adjusted_path = [
        np.array(get_joint_positions(robot, joints))
    ]  # Assumed the same as path[0] mod rotation
...
def draw_global_system(**kwargs):
    return draw_pose(Pose(), length=1.0, **kwargs)
