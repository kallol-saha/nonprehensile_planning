...
def create_ramp(half_extent=0.5):
    vertices = [
        Point(s1 * half_extent, s2 * half_extent, 0)
...
def at_rest(obj, tol=1e-3):
    ...
    return np.allclose(linear, 0, rtol=0, atol=tol) and np.allclose(
        angular, 0, rtol=0, atol=tol
    )
...
def simulate(
    controller=None, max_duration=INF, max_steps=INF, print_rate=1.0, sleep=None
):
    enable_gravity()
    dt = get_time_step()
    ...
    for step in irange(max_steps):
        ...
        if elapsed_time(last_print) >= print_rate:
            print(
                "Sim step: {} | Sim time: {:.3f} sec | Elapsed time: {:.3f} sec".format(
...
def main():
    ...
    parser.add_argument(
        "-shape", default="box", choices=["box", "sphere", "cylinder", "capsule"]
    )
