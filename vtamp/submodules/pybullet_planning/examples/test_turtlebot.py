...
def main(floor_width=2.0):
    ...
    set_camera_pose(
        camera_point=[1, -1, 1], target_point=unit_point()
    ...
    floor = create_box(
        w=floor_width, l=floor_width, h=0.001, color=TAN
    )  # Creates a tan box object for the floor
    set_point(floor, [0, 0, -0.001 / 2.0])  # Sets the [x,y,z] translation of the floor

    obstacle = create_box(w=0.5, l=0.5, h=0.1, color=RED)  # Creates a red box obstacle
    set_point(
        obstacle, [0.5, 0.5, 0.1 / 2.0]
    )  # Sets the [x,y,z] position of the obstacle
    print("Position:", get_point(obstacle))
    set_euler(
        obstacle, [0, 0, np.pi / 4]
    ...
    with LockRenderer():  # Temporarily prevents the renderer from updating for improved loading efficiency
        with HideOutput():  # Temporarily suppresses pybullet output
            robot = load_model(
                TURTLEBOT_URDF
            )  # TURTLEBOT_URDF | ROOMBA_URDF # Loads a robot from a *.urdf file
    ...
    theta_joint = joint_from_name(
        robot, "theta"
    )  # Looks up the robot joint named 'theta'
    joints = [x_joint, y_joint, theta_joint]

    base_link = link_from_name(
        robot, "base_link"
    )  # Looks up the robot link named 'base_link'
    ...
    for i in range(10):
        for handle in handles:
            remove_debug(handle)
        print("\nIteration: {}".format(i))
        x = random.uniform(-floor_width / 2.0, floor_width / 2.0)
        set_joint_position(robot, x_joint, x)  # Sets the current value of the x joint
        y = random.uniform(-floor_width / 2.0, floor_width / 2.0)
        set_joint_position(robot, y_joint, y)  # Sets the current value of the y joint
        yaw = random.uniform(-np.pi, np.pi)
        set_joint_position(
            robot, theta_joint, yaw
        )  # Sets the current value of the theta joint
        values = get_joint_positions(
            robot, joints
        )  # Obtains the current values for the specified joints
        print("Joint values: [x={:.3f}, y={:.3f}, yaw={:.3f}]".format(*values))

        world_from_robot = get_link_pose(
            robot, base_link
        )  # Returns the pose of base_link wrt the world frame
        (
            position,
            quaternion,
        ) = world_from_robot  # Decomposing pose into position and orientation (quaternion)
        ...
        print(
            "Base link orientation: [roll={:.3f}, pitch={:.3f}, yaw={:.3f}]".format(
                roll, pitch, yaw
            )
        )
        handles.extend(
        ...
        obstacle_from_robot = multiply(
            invert(world_from_obstacle), world_from_robot
        )  # Relative transformation from robot to obstacle

        robot_aabb = get_subtree_aabb(
            robot, base_link
        )  # Computes the robot's axis-aligned bounding box (AABB)
        (
            lower,
            upper,
        ) = robot_aabb  # Decomposing the AABB into the lower and upper extrema
        center = (lower + upper) / 2.0  # Computing the center of the AABB
