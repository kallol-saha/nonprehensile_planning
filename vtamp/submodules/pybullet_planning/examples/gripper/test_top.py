...
def main():
    ...
    with LockRenderer():
        ...
        add_line(
            Point(x=-TABLE_WIDTH / 2, z=block_z - BLOCK_SIDE / 2 + EPSILON),
            Point(x=+TABLE_WIDTH / 2, z=block_z - BLOCK_SIDE / 2 + EPSILON),
            color=RED,
        )
        set_camera_pose(
            camera_point=Point(y=-1, z=block_z + 1), target_point=Point(z=block_z)
        )
    ...
    y_grasp, x_grasp = get_top_grasps(
        block1, tool_pose=unit_pose(), grasp_length=0.03, under=False
    )
