...
from vtamp.heuristics import shelf_packing_heuristic as heuristic
from vtamp.heuristics import shelf_packing_goal as goal
...
def main(cfg, seed, overwrite: bool = False):
    ...
    output_dir = os.path.join("benchmarks", f"{cfg.benchmark_name}")
    ...
    shutil.copy(
        ...
        os.path.join(output_dir, "p2p_suggester.yaml"),
    )
    ...
    for folder in cfg.eval_folders:
        ...
        if os.path.exists(eval_data_dir):
            ...
            pattern = re.compile(r".*\.npz$")  # To match all npz files
            ...
            for data in tqdm(input_data):
                ...
                match = re.match(r"(\d+)_teleport_obj_points\.npz", data)
                if match:
                    data_id = int(match.group(1))
                else:
                    raise ValueError(f"Filename {data} does not match the expected pattern")

                pcd_data = np.load(os.path.join(eval_data_dir, data), allow_pickle=True)
                initial_pcd = pcd_data["clouds"]
                initial_pcd_seg = pcd_data["masks"]

                # Hard-code to segment out the shelf:
                num_objects = len(np.unique(initial_pcd_seg)) - 2
                initial_pcd_seg[initial_pcd_seg > num_objects] = -1
